...
The source for this shader is - https://www.shadertoy.com/view/ld2SzK
Big thanks to Otavio Good. We claim no ownership of this shader
*/
...
static vec3 saturate(vec3 a) { return clamp(a, 0.0, 1.0); }
static vec2 saturate(vec2 a) { return clamp(a, 0.0, 1.0); }
static float saturate(float a) { return clamp(a, 0.0, 1.0); }
...
vec3 RotateX(vec3 v, float rad)
{
  ...
  return vec3(v.x, vCos * v.y + vSin * v.z, -vSin * v.y + vCos * v.z);
  //else return new float3(x, vCos * y - vSin * z, vSin * y + vCos * z);
}
vec3 RotateY(vec3 v, float rad)
{
  ...
  float vSin = sin(rad);
  //if (RIGHT_HANDED_COORD)
  return vec3(vCos * v.x - vSin * v.z, v.y, vSin * v.x + vCos * v.z);
  //else return new float3(vCos * x + vSin * z, y, -vSin * x + vCos * z);
}
vec3 RotateZ(vec3 v, float rad)
{
  ...
  float vSin = sin(rad);
  //if (RIGHT_HANDED_COORD)
  return vec3(vCos * v.x + vSin * v.y, -vSin * v.x + vCos * v.y, v.z);
}


// This function basically is a procedural environment map that makes the sun
static vec3 sunCol = vec3(258.0, 208.0, 100.0) / 4255.0;//unfortunately, i seem to have 2 different sun colors. :(
...
static float nudge = 0.739513;	// size of perpendicular vector
...
static vec4 c00 = vec4(3.5, 2.0, 13.1, 0.0);	// start point
static vec4 c01 = vec4(12.5, 2.2, 17.0, 0.0);	// run up to canyon 2 before hole in large rock face
static vec4 c02 = vec4(21.5, 4.0, 8.1, 0.0);	// canyon 2 before hole in large rock face
static vec4 c03 = vec4(21.0, 5.0, 1.1, -0.5);	// before hole in large rock face
static vec4 c04 = vec4(17.8, 5.4, -0.2, 0.0);	// hole in large rock face
static vec4 c05 = vec4(14.7, 2.5, 1.4, 0.0);	// after hole in large rock face
static vec4 c06 = vec4(7.9, 2.3, -2.1, 0.0);
static vec4 c07 = vec4(0.5, -0.7, -3.5, 1.0);
static vec4 c08 = vec4(-3.0, -1.0, -3.5, 1.3);
static vec4 c09 = vec4(-3.5, -1.0, 4.0, 1.3);
static vec4 c10 = vec4(3.0, -0.7, 3.3, 0.8);
static vec4 c11 = vec4(3.5, -1.0, -4.75, 0.0);
static vec4 c12 = vec4(-6.0, -0.2, 1.0, 3.14);
static vec4 c13 = vec4(-6.0, -1.0, 5.5, 0.0);

static vec4 cXX = vec4(0.0, 3.0, 0.0, 0.0);

static float camPathOffset = 0.0;	// where to start on the camera path - parametric t var for catmull-rom spline
static vec3 camPos = vec3(0,0,0), camFacing;
static vec3 camLookat=vec3(0,0.0,0);
static float waterLevel = 1.5;
...
/*    if (iTime == 0.0)	// for debugging with manual camera
    ...
    //if (t <= maxDepth)
	{
        ...
        vec3 ballGlow = vec3(0.1, 0.97, 0.1) * abs(SpiralNoise3D(vec3(iTime*1.3,iTime*1.3,iTime*1.3)));
        ...
        if (length(pos) <= 2.01) texColor = vec3(1,1,1);
        ...
        finalColor = mix(vec3(1.0, 0.41, 0.41)*skyMultiplier + min(vec3(0.25,0.25,0.25),GetSunColorSmall(relVec, sunDir))*2.0*sunSet, finalColor, exp(-t*0.03));
	}
    else
    {
