...
vec3 tex3D(SamplerState tex, Texture2D tx, in vec3 p, in vec3 n) {

	n = max((abs(n) - 0.2)*7., 0.001); // max(abs(n), 0.001), etc.
	n /= (n.x + n.y + n.z);

	return (tx.Sample(tex, p.yz)*n.x + tx.Sample(tex, p.zx)*n.y + tx.Sample(tex, p.xy)*n.z).xyz;
}
...
float surfFunc(in vec3 p) {
	...
	return dot(tri(p*0.5 + tri(p*0.25).yzx), vec3(0.666, 0.666, 0.666));
}
...
vec3 doBumpMap(SamplerState tex, Texture2D tx, in vec3 p, in vec3 nor, float bumpfactor) {

	const float eps = 0.001;
	float ref = getGrey(tex3D(tex, tx, p, nor));
	vec3 grad = vec3(getGrey(tex3D(tex, tx, vec3(p.x - eps, p.y, p.z), nor)) - ref,
		getGrey(tex3D(tex, tx, vec3(p.x, p.y - eps, p.z), nor)) - ref,
		getGrey(tex3D(tex, tx, vec3(p.x, p.y, p.z - eps), nor)) - ref) / eps;
...
{
	// Screen coordinates.
	vec2 uv = vec2((fragCoord.x - iResolution.x*0.5) / iResolution.y, (1 - (fragCoord.y - iResolution.y*0.5)) / iResolution.y);
...
vec3 sceneCol = vec3(0.0, 0.0, 0.0);
...
if (dt < 0.005) {
	...
	if (sp.y < -(FH - 0.005)) sn = doBumpMap(iChannel1, tx1, sp*tSize1, sn, 0.025); // Floor.
	else sn = doBumpMap(iChannel0, tx0, sp*tSize0, sn, 0.025); // Walls.
	...
	vec3 texCol;
	if (sp.y < -(FH - 0.005)) texCol = tex3D(iChannel1, tx1, sp*tSize1, sn); // Floor.
	else texCol = tex3D(iChannel0, tx0, sp*tSize0, sn); // Walls.
