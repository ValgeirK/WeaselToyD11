...
// spheres
static const vec4 sc0 = vec4(  2.0, 0.5, 0.8, 0.5 );
static const vec4 sc1 = vec4( -6.0, 1.0,-4.0, 3.0 );
static const vec4 sc2 = vec4(-16.0, 1.0, 7.0, 4.0 );
static const vec4 sc3 = vec4(-25.0, 8.0, 0.0, 9.0 );
...
{
    float sh = min( min( min( softShadowSphere( pos, vec3(0.57703, 0.57703, 0.57703), sc0 ),
				              softShadowSphere( pos, vec3(0.57703, 0.57703, 0.57703), sc1 )),
				              softShadowSphere( pos, vec3(0.57703, 0.57703, 0.57703), sc2 )),
                              softShadowSphere( pos, vec3(0.57703, 0.57703, 0.57703), sc3 ));
...
float4 main( PS_INPUT input ) : SV_Target
{
	...
	if( t<100.0 )
	{
		...
		// shading		
		vec3 mate = vec3(0.0, 0.0, 0.0);
		if( id==0 ) 
            mate = vec3(1.0, 1.0, 1.0)*checkersTexture( uvw );
        else if( id==1 )
            mate = vec3(1.0, 1.0, 1.0)*checkersTextureGradBox( uvw, ddx_uvw, ddy_uvw );
        else if( id==2 )
            mate = vec3(1.0, 1.0, 1.0)*checkersTextureGradTri( uvw, ddx_uvw, ddy_uvw );
        ...
        // fog		
        col = mix( col, vec3(0.9, 0.9, 0.9), 1.0-exp( -0.0001*t*t ) );
	}
